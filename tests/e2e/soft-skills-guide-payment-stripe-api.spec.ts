...
/**
 * E2E Test: Soft Skills Guide - Pago con Stripe (Validación avanzada)
 *
 * User Journey:
...
test.describe('Soft Skills Guide - Pago con Stripe (Validación Stripe y Backend)', () => {
  test('debe completar el pago y validar integración', async ({ page, request }) => {
    ...
    const testEmail = `softskills-stripe-${Date.now()}@example.com`
    ...
    // Consultar backend para validar el registro del pago (soft_skills_orders)
    // Suponiendo endpoint: /api/soft-skills-guide/order/[payment_intent_id]
    const backendResponse = await request.get(`/api/soft-skills-guide/order/${sessionId}`)
