...
export async function GET(request: NextRequest) {
  try {
    ...
    const format = request.url.includes('/pdf') ? 'pdf' : 'epub'
    ...
    // Get file path
    const fileName = `soft-skills-guide.${format}`
    ...
    if (!fs.existsSync(filePath)) {
      ...
      return new NextResponse(
        `Este es un archivo ${format.toUpperCase()} de placeholder para la Guía de Soft Skills IT.\n\nEn producción, aquí se serviría el archivo real.`,
        {
          headers: {
